...
procedure Update_Error_Notices (
   Error_Notices_Data : in out XOS_Types.
				  XOS_ERROR_NOTICES_MONITOR_DATA_REC_PTR) is
   ...
   Error_Entry          : OS_Data_Types.ERROR_QUEUE_ENTRY_TYPE;
   ...
   function "="(Left, Right: XOS_Types.
     XOS_ERROR_NOTICES_MONITOR_DATA_REC_PTR) return BOOLEAN
       renames XOS_Types."=";
...
begin
    ...
    if ((Error_Notices_Data /= NULL) and then (Error_Notices_Data.
	 ...
	 Client_Data => XOS_Types.
	   XOS_ERROR_NOTICES_MONITOR_DATA_REC_PTR_to_XtPOINTER(
	     Error_Notices_Data));
	  ...
	  if (Error_Entry.Detonation_Flag) then
	     Error_String := new STRING'(Utilities.Time_To_String (
	       Error_Entry.Occurrence_Time) & K_SEPARATION_STRING
		 & XOS_Types.K_Detonated_Prematurely_Error_Message
	           & OS_Error_Messages.K_Error_Message(Error_Entry.Error).Text
	             & ASCII.LF & ASCII.NUL);
	  else
	     Error_String := new STRING'(Utilities.Time_To_String (
	       Error_Entry.Occurrence_Time) & K_SEPARATION_STRING
