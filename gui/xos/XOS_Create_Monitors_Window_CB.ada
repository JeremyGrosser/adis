...
--
-- PURPOSE:
--   This procedure creates the XOS Monitors window.
--
...
procedure Create_Monitors_Window_CB (
   ...
   function "=" (left, right: XOS_Types.XOS_MONITORS_DATA_REC_PTR)
   ...
   function XOS_PARM_DATA_REC_PTR_to_Integer is new 
     Unchecked_Conversion (Source => XOS_Types.XOS_PARM_DATA_REC_PTR,
                           Target => INTEGER);
   function XOS_MONITORS_DATA_REC_PTR_to_Integer is new 
     Unchecked_Conversion (Source => XOS_Types.XOS_MONITORS_DATA_REC_PTR,
                           Target => INTEGER);
...
begin
   ...
   --
   if ((XOS_Data.Monitors_Data /= NULL) and then 
     (XOS_Data.Monitors_Data.Shell /= Xt.XNULL)) then

      Xt.ManageChild (XOS_Data.Monitors_Data.Shell);
      Xt.Popup (XOS_Data.Monitors_Data.Shell, Xt.GrabNone);
      Xlib.MapRaised (
	Xt.GetDisplay (XOS_Data.Monitors_Data.Shell), 
	Xt.GetWindow  (XOS_Data.Monitors_Data.Shell));
   ...
   --
      ...
      --
      XOS_Data.Monitors_Data := new XOS_Types.XOS_MONITORS_DATA_REC'(
	Shell                        => Xt.XNULL,
	Title                        => Xt.XNULL,
	...
	Error_History                => NULL);
      XOS_Data.Monitors_Data.Error_History := new
	XOS_Types.XOS_MON_ERROR_HISTORY_DATA_REC'(
	   Shell                         => Xt.XNULL,
	...
	XOS.K_Monitors_Window_Width);
      Argcount := Argcount + 1;
      Xt.SetArg (Arglist (Argcount), Xtdef.Nheight,
	XOS.K_Monitors_Window_Height);
      Argcount := Argcount + 1;
      Xt.SetArg (Arglist (Argcount), Xmdef.NdeleteResponse, 
	INTEGER(Xm.DO_NOTHING));
      XOS_Data.Monitors_Data.Shell := Xt.CreatePopupShell (K_Title,
	...
	XOS_Data.Monitors_Data.Shell), "WM_DELETE_WINDOW", False);
      Xm.AddWMProtocolCallback (XOS_Data.Monitors_Data.Shell,
	WM_Delete_Window, XOS.Close_Window_CB'address,
	XOS_Data.Monitors_Data.Shell);
	...
	XOS_Data.Monitors_Data.Shell, Arglist, Argcount);
	...
	" Cancel ");
	  ...
	  XOS.Close_Window_CB'address, XOS_Data.Monitors_Data.Shell);
      ...
      Xt.SetArg (Arglist (Argcount), Xmdef.NrightOffset, 
        Motif_Utilities.Widget_Separation);
      XOS_Data.Monitors_Data.Description := Xt.CreateManagedWidget (
	...
	Parameter_Selection_Scrolled_Window, Arglist, Argcount);
      ...
      --  XOS.Create_Monitors_Panel_Entities'address, 
      --  XOS_Types.XOS_MONITORS_DATA_REC_PTR_to_XtPOINTER(
      --    XOS_Data.Monitors_Data));
--
...
--	XOS.Create_Monitors_Panel_Gateway'address, 
--        XOS_Types.XOS_MONITORS_DATA_REC_PTR_to_XtPOINTER(
--	  XOS_Data.Monitors_Data));
	...
	XOS.Create_Monitors_Panel_Errors'address, 
        XOS_Types.XOS_MONITORS_DATA_REC_PTR_to_XtPOINTER(
	  XOS_Data.Monitors_Data));
	...
	Parameter_Selection_Scrolled_Window);
      ...
      --
      XOS_Data.Monitors_Data.Title := Motif_Utilities.Create_Label (
	Work_Form, K_Welcome_Message);
      ...
      Xt.SetArg (Arglist (Argcount), Xmdef.NrightOffset, 
        Motif_Utilities.Widget_Separation);
      Xt.SetValues (XOS_Data.Monitors_Data.Title, Arglist, Argcount);
      ...
      Xt.SetArg (Arglist (Argcount), Xmdef.NrightOffset, 
        Motif_Utilities.Widget_Separation);
      XOS_Data.Monitors_Data.Parameter_Scrolled_Window :=
	Xm.CreateScrolledWindow (Work_Form, "Parameter_Scrolled_Window",
	  Arglist, Argcount);
      Xt.ManageChild (XOS_Data.Monitors_Data.Parameter_Scrolled_Window);
