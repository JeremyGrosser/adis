...
procedure Create_Error_Notices_Window_CB (
   ...
   K_Error_Notices_Help_String : constant STRING :=
     "This is a list of XOS errors as they occur.";
   ...
   --
   function XOS_PARM_DATA_REC_PTR_to_Integer
     is new Unchecked_Conversion (
       Source => XOS_Types.XOS_PARM_DATA_REC_PTR,
       Target => INTEGER);

   function XOS_ERROR_NOTICES_MONITOR_DATA_REC_PTR_to_INTEGER
     is new Unchecked_Conversion (
       Source => XOS_Types.XOS_ERROR_NOTICES_MONITOR_DATA_REC_PTR,
       Target => INTEGER);
...
begin
   ...
   --
   if ((XOS_Data.Error_Notices_Data /= NULL) and then 
     (XOS_Data.Error_Notices_Data.Shell /= Xt.XNULL)) then

      Xt.ManageChild (XOS_Data.Error_Notices_Data.Shell);
      Xt.Popup (XOS_Data.Error_Notices_Data.Shell, Xt.GrabNone);
      Xlib.MapRaised (
	Xt.GetDisplay (XOS_Data.Error_Notices_Data.Shell), 
	Xt.GetWindow  (XOS_Data.Error_Notices_Data.Shell));
   ...
   --
      ...
      --
      XOS_Data.Error_Notices_Data
	:= new XOS_Types.XOS_ERROR_NOTICES_MONITOR_DATA_REC'(
	  Shell                       => Xt.XNULL,
	  Title                       => Xt.XNULL,
	...
	XOS.K_Error_Notices_Window_Width);
      Argcount := Argcount + 1;
      Xt.SetArg (Arglist (Argcount), Xtdef.Nheight,
	XOS.K_Error_Notices_Window_Height);
      Argcount := Argcount + 1;
      Xt.SetArg (Arglist (Argcount), Xmdef.NdeleteResponse, 
	INTEGER(Xm.DO_NOTHING));
      XOS_Data.Error_Notices_Data.Shell := Xt.CreatePopupShell (K_Title,
	...
	XOS_Data.Error_Notices_Data.Shell), "WM_DELETE_WINDOW", False);
      Xm.AddWMProtocolCallback (XOS_Data.Error_Notices_Data.Shell,
	WM_Delete_Window, XOS.Close_Window_CB'address,
	XOS_Data.Error_Notices_Data.Shell);
	...
	XOS_Data.Error_Notices_Data.Shell, Arglist, Argcount);
	...
	XOS.Close_Window_CB'address, 
      ...
      Xt.SetArg (Arglist (Argcount), Xmdef.NrightOffset, 
        Motif_Utilities.Widget_Separation);
      XOS_Data.Error_Notices_Data.Description := Xt.CreateManagedWidget (
	...
	Arglist, Argcount);
      ...
      --
      XOS.Error_Notices_Text_Position          := Xm.TEXTPOSITION(0);
      XOS_Data.Error_Notices_Data.Error_Notices_Count := 0;

      --
      -- Create the Error Notices text widget.
      --
      Argcount := 0;
      XOS_Data.Error_Notices_Data.Error_Notices_Text  :=
	...
	INTEGER(Xm.ATTACH_FORM));
      ...
      Xt.SetValues (Xt.Parent(XOS_Data.Error_Notices_Data.Error_Notices_Text),
	Arglist, Argcount);
...
--      Argcount := Argcount + 1;
      ...
      Xt.SetValues (XOS_Data.Error_Notices_Data.Error_Notices_Text,
	Arglist, Argcount);
	...
	Parent             => XOS_Data.Error_Notices_Data.Error_Notices_Text,
	Help_Text_Widget   => XOS_Data.Error_Notices_Data.Description,
	Help_Text_Message  => K_Error_Notices_Help_String);

      --
      -- Install the TimeProc to update the Error Notices window
      --
      Update_Error_Notices (XOS_Data.Error_Notices_Data);


      --
      -- Manage and display window
      --
      Xt.ManageChild (XOS_Data.Error_Notices_Data.Error_Notices_Text);
      Xt.ManageChild (XOS_Data.Error_Notices_Data.Shell);
      Xt.Popup (XOS_Data.Error_Notices_Data.Shell, Xt.GrabNone);
