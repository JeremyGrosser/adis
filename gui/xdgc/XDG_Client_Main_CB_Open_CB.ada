...
procedure Open_CB (
   ...
   K_Client_Name  : constant STRING  := DG_Client_GUI.Interface.
     Client_Name.Name(DG_Client_GUI.Interface.Client_Name.Name'first..
       DG_Client_GUI.Interface.Client_Name.Name'first + DG_Client_GUI.
	 Interface.Client_Name.Length - 1);
   K_Prompt_Title : constant STRING  := "Open Prompt ("
     & K_Client_Name & ")" & ASCII.NUL;
   K_Dialog_Title : constant STRING  := "Config File Open Dialog ("
     & K_Client_Name & ")" & ASCII.NUL;
...
begin
    ...
    if (Motif_Utilities.Prompt_User (Parent, Xm.DIALOG_QUESTION,
	  ...
	  & ASCII.LF & "(All data currently entered into the XDG Client GUI "
	    & "will be overwritten...)",
	 ...
	 XDG_Client_Types.K_Config_File_Base_Directory 
	   & XDG_Client_Types.K_Config_File_Mask);
	 ...
	 XDG_Client_Main_CB.Open_Config_File_FSB_CB'address,
