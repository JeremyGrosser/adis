...
--
-- PURPOSE:
--   -This package holds the code for the basic XDG Client application.
--
-- EFFECTS:
--   None.
--
...
procedure Main is
   ...
   K_App_Name  : constant STRING := "XDG Client";

   -- Constant application label (description).
   K_App_Label_String : constant STRING :=
     "ADIS X-based DIS Gateway Client Interface";
   ...
   App_Title    : Utilities.ASTRING := NULL;
   ...
   --
   XDG_Data : XDG_Client_Types.XDG_DATA_REC_PTR;
   XDG_Main_Window_Name : Utilities.ASTRING;

   --
   -- Declare DG Client data
   --
   ...
   function XDG_PARM_DATA_REC_PTR_to_Integer is new
     Unchecked_Conversion (Source => XDG_Client_Types.XDG_DATA_REC_PTR,
			   Target => INTEGER);
   function ASTRING_to_XtPOINTER is new
     Unchecked_Conversion (Source => Utilities.ASTRING,
			   Target => Xt.POINTER);
   procedure Free
...
begin
   ...
   --
   XDG_Data := new XDG_Client_Types.XDG_DATA_REC'(
   ...
   --
   XDG_Client_Main_CB.App_Context := Xt.CreateApplicationContext;

   --
   -- Sets the locale for internationalization to the default.
   --
   XtR5.SetLanguageProc (XDG_Client_Main_CB.App_Context, Xt.XNULL, Xt.XNULL);
   ...
   --
   XtAppSetFallbackResources (XDG_Client_Main_CB.App_Context,
   ...
   Xt.OpenDisplay (XDG_Client_Main_CB.App_Context, K_Null_String,
   ...
   Xm.AddWMProtocolCallback (Toplevel, WM_Delete_Window,
     XDG_Client_Main_CB.Quit_CB'address, Xt.XNULL);
   ...
   Motif_Utilities.Build_Menu_Item (
     ...
     Accel_text  => "",        -- "Alt+O",
     Callback    => XDG_Client_Main_CB.Open_CB'address,
     CB_Data     => XDG_Client_Types.XDG_DATA_REC_PTR_to_XtPOINTER(XDG_Data),
     Subitems    => NULL,
     Item_Widget => NULL,
   ...
   Motif_Utilities.Build_Menu_Item (
     ...
     Accel_text  => "",        -- "Alt+S",
     Callback    => XDG_Client_Main_CB.Save_CB'address,
     CB_Data     => XDG_Client_Types.XDG_DATA_REC_PTR_to_XtPOINTER(XDG_Data),
     Subitems    => NULL,
     Item_Widget => NULL,
   ...
   Motif_Utilities.Build_Menu_Item (
     ...
     Accel_text  => "Alt+Q",
     Callback    => XDG_Client_Main_CB.Quit_CB'address,
     CB_Data     => Xt.XNULL,
     Subitems    => NULL,
     Item_Widget => NULL,
   ...
   Motif_Utilities.Build_Menu_Item (
     ...
     Accelerator => "Alt<Key>S",
     Accel_text  => "Alt+S",
     Callback    => XDG_Client.Create_Set_Parms_Window_CB'address,
     CB_Data     => XDG_Client_Types.XDG_DATA_REC_PTR_to_XtPOINTER(XDG_Data),
     Subitems    => NULL,
     Item_Widget => NULL,
   ...
   Motif_Utilities.Build_Menu_Item (
     ...
     Accel_text  => "Alt+M",
     Callback    => XDG_Client.Create_Monitors_Window_CB'address,
     CB_Data     => XDG_Client_Types.XDG_DATA_REC_PTR_to_XtPOINTER(XDG_Data),
     Subitems    => NULL,
     Item_Widget => NULL,
   ...
   Motif_Utilities.Build_Menu_Item (
     ...
     Accel_text  => "Alt+N",
     Callback    => XDG_Client.Create_Error_Notices_Window_CB'address,
     CB_Data     => XDG_Client_Types.XDG_DATA_REC_PTR_to_XtPOINTER(XDG_Data),
     Subitems    => NULL,
     Item_Widget => NULL,
   ...
   --
   -- Create the XDG Client menu.
   --
   Motif_Utilities.Build_Menu (
     Parent           => Main_Menubar,
     Menu_Type        => Xm.MENU_PULLDOWN,
     Menu_Title       => "XDG Client",
     Menu_Mnemonic    => 'X',
     Menu_Sensitivity => TRUE,
     Items            => DG_Menu,
...
--      -- vvv TEMP vvv
--      --
--      Text_IO.Put_Line("Unable to map Client/GUI interface.");
--      Text_IO.Put_Line("XDG mapping Client/GUI interface...");
--      DG_Client_GUI.Map_Interface(
--	Create_Interface => TRUE,
--	Status           => DG_Status_Flag);
--      if (DG_Status.Failure(DG_Status_Flag)) then
--         Text_IO.Put_Line("XDG unable to map Client/GUI interface");
--         raise DG_Failure;
--      else
--	 XDG_Client.Self_Mapped := True;
--      end if;
--      --
--      -- ^^^ TEMP ^^^
--      --
   ...
   --
   -- Assign Application Title to the main window.
   --
   App_Title := new STRING'(K_App_Name & " ("
     & DG_Client_GUI.Interface.Client_Name.Name(
       DG_Client_GUI.Interface.Client_Name.Name'first..
	 DG_Client_GUI.Interface.Client_Name.Name'first
	   + DG_Client_GUI.Interface.Client_Name.Length - 1) & ")" 
	     & ASCII.NUL);
   Argcount := 0;
   Argcount := Argcount + 1;
   Xt.SetArg (Arglist (Argcount), Xmdef.Ntitle           , App_Title.all);
   Xt.SetValues (Toplevel, Arglist, Argcount);
   Free (App_Title);

   --
   -- Allow X to Loop and process events indefinitely.
   --
   Xt.AppMainLoop (XDG_Client_Main_CB.App_Context);
...
exception
   ...
   when DG_Failure =>
      Text_IO.Put_Line("Unable to map Client/GUI interface.");
      Text_IO.Put_Line("Please run DG Client; DG Client GUI Exiting...");

   when OTHERS =>
      Text_IO.Put_Line(
	"Unknown DG Client GUI Error; DG Client GUI Exiting...");
