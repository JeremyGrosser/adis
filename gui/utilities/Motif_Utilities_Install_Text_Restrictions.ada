...
procedure Install_Text_Restrictions(
   ...
   Characters_Count : in     INTEGER) is
...
begin
   ...
   --
   -- Extract the string from the parent widget.
   --
   if (Parent /= Xt.XNULL) then
	...
	Text_Type_Enum => Text_Type);
      ...
      case Text_Type is
	   ...
	   | TEXT_NUMERIC_INTEGER_NONNEGATIVE | TEXT_NUMERIC_BINARY
	     | TEXT_NUMERIC_HEXADECIMAL =>
	      Text_Restrictions_Ptr.Minimum_Valid_Integer
		:= INTEGER'first;
	      Text_Restrictions_Ptr.Maximum_Valid_Integer
		:= INTEGER'last;

         when TEXT_NUMERIC_FLOAT | TEXT_NUMERIC_FLOAT_POSITIVE
	   | TEXT_NUMERIC_FLOAT_NONNEGATIVE =>
	      Text_Restrictions_Ptr.Minimum_Valid_Float := -FLOAT'large;
	      Text_Restrictions_Ptr.Maximum_Valid_Float := FLOAT'large;
