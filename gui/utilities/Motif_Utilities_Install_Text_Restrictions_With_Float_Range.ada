	...
	Text_Type_Enum => Text_Type);
	 ...
	 when TEXT_NUMERIC_INTEGER | TEXT_NUMERIC_INTEGER_POSITIVE
	   | TEXT_NUMERIC_INTEGER_NONNEGATIVE =>
		...
		:= INTEGER(Minimum_Float);
	      Text_Restrictions_Ptr.Maximum_Valid_Integer
		:= INTEGER(Maximum_Float);
	 when TEXT_NUMERIC_FLOAT | TEXT_NUMERIC_FLOAT_POSITIVE
	   | TEXT_NUMERIC_FLOAT_NONNEGATIVE =>
	      Text_Restrictions_Ptr.Minimum_Valid_Float := Minimum_Float;
	      Text_Restrictions_Ptr.Maximum_Valid_Float := Maximum_Float;
         when OTHERS =>
            Text_Restrictions_Ptr.Minimum_Valid_Float := Minimum_Float;
            Text_Restrictions_Ptr.Maximum_Valid_Float := Maximum_Float;
      end case;
...
end Install_Text_Restrictions_With_Float_Range;
