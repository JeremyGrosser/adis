...
procedure Update_Error_History (
   ...
   function "="(Left, Right: Xt.WIDGET) return BOOLEAN
     renames Xt."=";
   function "="(Left, Right:
     XDG_Server_Types.XDG_MON_ERROR_HISTORY_DATA_REC_PTR) return BOOLEAN 
       renames XDG_Server_Types."=";
    ...
    if ((Error_History_Data /= NULL) and then
	 ...
	 then
	  ...
	  for Index in DG_Status.STATUS_TYPE loop
	     ...
	     if (DG_Server_GUI.Interface.Error_Monitor.Error_History(Index).
		...
		Motif_Utilities.Set_Labelstring(
		  Error_History_Data.History(Index).Time,
		    Utilities.Time_To_String (DG_Server_GUI.Interface.
		...
		Motif_Utilities.Set_Labelstring(
		  Error_History_Data.History(Index).Occurrences,
		    INTEGER'image(DG_Server_GUI.Interface.Error_Monitor.
