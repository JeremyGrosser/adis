...
procedure Initialize_Panel_Error (
   Error : in     XDG_Server_Types.XDG_ERROR_PARAMETERS_DATA_REC) is
...
begin
   ...
   if (Error.GUI_Error_Reporting /= Xt.XNULL) then
      if (DG_Server_GUI.Interface.Error_Parameters.Error_Monitor_Enabled
	= XDG_Server.K_Enabled) then
	 XDG_Server.GUI_Error_Reporting_Flag := XDG_Server.K_Enabled;
	 Motif_Utilities.Set_LabelString (Xm.OptionButtonGadget(
	   Error.GUI_Error_Reporting), K_Enabled_String);
      else
	 XDG_Server.GUI_Error_Reporting_Flag := XDG_Server.K_Disabled;
	 Motif_Utilities.Set_LabelString (Xm.OptionButtonGadget(
   ...
   if (Error.Error_Logging /= Xt.XNULL) then
      if (DG_Server_GUI.Interface.Error_Parameters.Error_Log_Enabled
	= XDG_Server.K_Enabled) then
	 XDG_Server.Error_Logging_Flag := XDG_Server.K_Enabled;
	 Motif_Utilities.Set_LabelString (Xm.OptionButtonGadget(
	   Error.Error_Logging), K_Enabled_String);
      else
	 XDG_Server.Error_Logging_Flag := XDG_Server.K_Disabled;
	 Motif_Utilities.Set_LabelString (Xm.OptionButtonGadget(
   ...
   if (Error.Error_Logfile /= Xt.XNULL) then
      ...
      if DG_Server_GUI.Interface.Error_Parameters.Error_Log_File.Length > 0
	then

         Xm.TextFieldSetString (Error.Error_Logfile, DG_Server_GUI.Interface.
           Error_Parameters.Error_Log_File.Name(DG_Server_GUI.Interface.
	     Error_Parameters.Error_Log_File.Name'first..DG_Server_GUI.
	       Interface.Error_Parameters.Error_Log_File.Name'first + 
		 DG_Server_GUI.Interface.Error_Parameters.
