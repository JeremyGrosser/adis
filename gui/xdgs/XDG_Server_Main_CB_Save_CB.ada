...
--   XDG Server screens into a configuration file.
--
...
procedure Save_CB (
   ...
   K_Dialog_Title : constant STRING
...
begin
    ...
    --
    if (DG_Server_GUI.Interface.Configuration_File.Length > 0) then

       Xm.TextSetString (Xm.FileSelectionBoxGetChild (File_Selection_Box,
	 Xm.DIALOG_TEXT), DG_Server_GUI.Interface.Configuration_File.Name(
	   DG_Server_GUI.Interface.Configuration_File.Name'first..
	     DG_Server_GUI.Interface.Configuration_File.Name'first
	       + DG_Server_GUI.Interface.Configuration_File.Length - 1));
    else
       Xm.TextSetString (Xm.FileSelectionBoxGetChild (File_Selection_Box,
	 Xm.DIALOG_TEXT), XDG_Server_Types.K_Default_Config_Filename);
    end if;
    ...
    Save_Warning_Label := Motif_Utilities.Create_Label (File_Selection_Box,
      XDG_Server.K_Save_Warning_String);
    Argcount := 0;
    Argcount := Argcount + 1;
    ...
    Xt.AddCallback (File_Selection_Box, Xmdef.NokCallback,
      XDG_Server_Main_CB.Save_Config_File_FSB_CB'address,
