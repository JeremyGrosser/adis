...
--
-- ORIGINATION DATE:   August 3, 1994
--
-- PURPOSE:
...
procedure Initialize_Panel_DG_Parameters (
   DG_Parameters : in     XDG_Server_Types.XDG_DG_PARAMETERS_DATA_REC) is
...
begin
   ...
   --
   -- Initialize Timeslice widget
   --
   if (DG_Parameters.Timeslice /= Xt.XNULL) then
      Utilities.Integer_To_String (
	Integer_Value => INTEGER(
	  DG_Server_GUI.Interface.Timeslice),
	Return_String => Temp_String);
      Xm.TextFieldSetString (DG_Parameters.Timeslice, Temp_String);
   end if;
   ...
   if (DG_Parameters.Max_Entities /= Xt.XNULL) then
      Utilities.Integer_To_String (
	Integer_Value => INTEGER(DG_Server_GUI.Interface
