...
--=============================================================================
--
-- Unit Name:          Entity_To_Geocentric_Conversion
--
-- File Name:          CC__Entity_To_Geocentric_Conversion.ada
--
...
--
--   Use the Euler Angle's trigometric values to calculate the 
...
procedure Entity_To_Geocentric_Conversion (
   ...
   -- Define a local variable for the entity coordinates that is compatible
   -- with the geocentric coordinate system. 
   Converted_Entity_Coordinates : DIS_Types.A_WORLD_COORDINATE := (
   ...
   Trig                         : DL_Math.EULER_TRIG_64_BIT := (OTHERS => 0.0);
...
begin -- Entity_To_Geocentric_Conversion
   ...
   Geocentric_Coordinates.X := 
     ...
     + Converted_Entity_Coordinates.X * (Trig(DL_Math.COS_PSI) 
         * Trig(DL_Math.COS_THETA)) 
 
     + Converted_Entity_Coordinates.Y * (Trig(DL_Math.COS_PSI_SIN_PHI) 
         * Trig(DL_Math.SIN_THETA) - Trig(DL_Math.SIN_PSI_COS_PHI))

     + Converted_Entity_Coordinates.Z * (Trig(DL_Math.COS_PSI_COS_PHI) 
   ...
   Geocentric_Coordinates.Y := 
     ...
     + Converted_Entity_Coordinates.X * (Trig(DL_Math.SIN_PSI) 
         * Trig(DL_Math.COS_THETA))
      
     + Converted_Entity_Coordinates.Y * (Trig(DL_Math.SIN_PSI_SIN_PHI) 
         * Trig(DL_Math.SIN_THETA) + Trig(DL_Math.COS_PSI_COS_PHI))
      
     + Converted_Entity_Coordinates.Z * (Trig(DL_Math.SIN_PSI_COS_PHI)
    ...
    Geocentric_Coordinates.Z := 
      ...
      - Converted_Entity_Coordinates.X * Trig(DL_Math.SIN_THETA) 
        
      + Converted_Entity_Coordinates.Y * Trig(DL_Math.COS_THETA)
          * Trig(DL_Math.SIN_PHI)

      + Converted_Entity_Coordinates.Z * Trig(DL_Math.COS_THETA)
          * Trig(DL_Math.COS_PHI);
...
exception
   ...
   when OTHERS       => 
      Status := DL_Status.ENT_GCC_FAILURE;
        
end Entity_To_Geocentric_Conversion;
