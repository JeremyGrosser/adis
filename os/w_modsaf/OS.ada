...
--     Gateway_Interface, Munition, OS_GUI, OS_Simulation_Types, OS_Status,
--     OS_Timer, and Text_IO.
--
-- EXCEPTIONS :
--
-- PORTABILITY ISSUES :
...
begin -- OS
   ...
   if Status_DG /= DG_Status.SUCCESS then
      Text_IO.Put_Line("An error occurred in DG_Client.Initialize_Client");
   end if;
   ...
   if Returned_Status /= OS_Status.SUCCESS then
      Text_IO.Put_Line("An error occurred in Initialize_Simulation");
   end if;
...
exception
   when CT_ERROR | DG_ERROR | ST_ERROR =>
      Errors.Report_Error(
        Detonated_Prematurely => FALSE,
        Error                 => Returned_Status);
...
--
-- 13 NOV 94 -- KJN:  Changed error reporting for Initialize_Client and
--           Initialize_Simulation to print the error directly to the screen
